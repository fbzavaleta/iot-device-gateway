...
void http_SendReceive(void * pvParameter)
{
	...
	const char * msg_post = \
        ...
        "Connection: close\n"
        "X-THINGSPEAKAPIKEY: H2NJFSMMH7LIN9ZQ\n"		                    
        "Content-Type: application/x-www-form-urlencoded\n"
        "content-length: ";
		
	char databody[50];
  	sprintf( databody, "{H2NJFSMMH7LIN9ZQ&field1=%d}", xSocket->distance);
...
void app_main(void)
{
	...
	if ( (xQueue_timepout_x = xQueueCreate(25, sizeof(adc_struct))) ==NULL)
	{
         ESP_LOGI( TAG, "error - Nao foi possivel alocar a quee.\r\n" );  
         return;  
	}

	if( (XQuee_navigation = xQueueCreate( 10, sizeof(adc_struct)) ) == NULL )
	{
	...
	if( (XQuee_comunications = xQueueCreate( 10, sizeof(adc_struct)) ) == NULL )
	{
