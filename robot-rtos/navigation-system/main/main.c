...
typedef struct {
    uint32_t meters_distance;
    uint32_t centimeters_distance;
    TaskHandle_t taskHandle;
} xpto;
...
void ultrasonic()
{
	...
	while (true) {
		...
		if (res != ESP_OK) {
			...
			distance_ultrasonic.centimeters_distance = distance;
			distance_ultrasonic.meters_distance = distance / 100.0;
		}
